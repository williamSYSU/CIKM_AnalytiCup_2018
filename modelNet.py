...
class MatchSRNN(nn.Module):
    ...
    def forward(self, input1, input2):
        ...
        s = F.max_pool2d(F.LeakyReLU(self.conv1(s)), (2, 2))
        s = F.max_pool2d(F.LeakyReLU(self.conv2(s)), (2, 2))
        s = F.LeakyReLU(self.c1_linear(s.view(-1, self.num_flat_features(s))))
        s = F.LeakyReLU(self.c2_linear(s))
        print("s:", s)
